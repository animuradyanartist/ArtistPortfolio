...
- November 24, 2025 (Production Gallery Upload Fix): Fixed gallery photo uploads failing in production environment. Implemented client-side image compression to ensure gallery photos stay under Replit's 5MB request body limit for published apps. Images are now automatically resized (max 1920px width) and compressed to JPEG at 80% quality before upload, with additional compression to 60% if needed. This ensures gallery photos work reliably in both development and production environments while maintaining good image quality.
